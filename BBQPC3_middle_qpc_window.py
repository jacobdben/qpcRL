...
start = -0.8
window_size = 0.8
window_bounds=(-0.4,0.8) #this gives a window of 800mv, that can be moved between 0V and -2V
...
def func_to_minimize(x,table): #x len 8
    ...
    window_move,penalty2=simple_new_point([x[-1]],bounds=window_bounds)[0][0]
    voltages_send=vals+window_move
    ...
    table['measurements'][key]={'loss':val+penalty*pfactor,'staircase':result,'x':x.tolist(),'voltages':voltages.tolist(),'dataid':dataid,'deriv_metric':val}
...
xbest,es,run_id=optimize_cma(func_to_minimize,dat,start_point=np.zeros(8),stop_time=16*3600,options={'tolx':1e-3})
