...
class regression_network():
    def __init__(self,input_layer_dim=10,data_dim=9,activation_func='tanh',hidden_layers=1,hidden_layer_neurons=100):
        ...
        for i in range(hidden_layers):
            self.model.add(Dense(hidden_layer_neurons,activation=activation_func))
        self.model.add(Dense(1, kernel_initializer='normal'))
        # Compile model
        
        
        #make loss function used when calculating loss on prediction
        self.loss_func=MeanSquaredError()
        self.model.compile(loss=self.loss_func, optimizer='adam')
        
    
    
    def train_model(self,X_train,Y_train,plot_history=True,ax=None,validation_data=None,epochs=10,verbose=0):
        now = datetime.now()
        ...
        if validation_data==None:
            ...
            history=self.model.fit(X_train,Y_train,epochs=epochs,verbose=0,validation_data=validation_data)
